...
string Library::get_matched_left_read_name(){
    return tmp_dir + "/output_matched_left_" + int2str(lib_idx+1) + "." + this->file_extension;
}

string Library::get_matched_right_read_name(){
    return tmp_dir + "/output_matched_right_" + int2str(lib_idx+1) + "." + this->file_extension;
}
string Library::get_matched_left_read_name(int round){
    return tmp_dir + "/output_matched_left_" + int2str(round) + "_" + int2str(lib_idx+1) + "." + this->file_extension;
}

string Library::get_matched_right_read_name(int round){
    return tmp_dir + "/output_matched_right_" + int2str(round) + "_" + int2str(lib_idx+1) + "." + this->file_extension;
}
string Library::get_matched_left_read_name(int round, int idx){
    return tmp_dir + "/output_matched_left_" + int2str(round) + "_" + int2str(lib_idx+1) + "_" + int2str(idx) + "." + this->file_extension;
}

string Library::get_matched_right_read_name(int round, int idx){
    if (paired_end)
	    return tmp_dir + "/output_matched_right_" + int2str(round) + "_" + int2str(lib_idx+1) + "_" + int2str(idx) + "." + this->file_extension;
    return "";
}

string Library::get_joined_read_name(int round, int idx, int file_type){
	string extension = (file_type == FORMAT_FASTQ)? "fastq" : "fasta";
    return tmp_dir + "/output_matched_joined_" + int2str(round) + "_" + int2str(lib_idx+1) + "_" + int2str(idx) + "." + extension;
}
